...
# --- ОПРЕДЕЛЕНИЕ ПУТЕЙ ДЛЯ PYINSTALLER ---
if getattr(sys, 'frozen', False):
    # Если запущено как скомпилированный .exe (PyInstaller)
    TEMP_DIR = sys._MEIPASS # Временная папка с вшитыми файлами (--add-data)
    APP_DIR = os.path.dirname(sys.executable) # Папка, где лежит сам .exe
else:
    # Если запущено как обычный скрипт main.py
    APP_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    TEMP_DIR = APP_DIR

# Вшитые ресурсы (иконки, заглушка) всегда берем из временной папки
ICON_ICO = os.path.join(TEMP_DIR, "resources", "icon.ico")
ICON_PNG = os.path.join(TEMP_DIR, "resources", "icon.png")
PICTURE_PNG = os.path.join(TEMP_DIR, "resources", "picture.png")

# Стартовые фото берем из вшитой папки, а НОВЫЕ сохраняем рядом с .exe
PHOTOS_DIR_TEMP = os.path.join(TEMP_DIR, "resources", "photos")
PHOTOS_DIR = os.path.join(APP_DIR, "resources", "photos")
os.makedirs(PHOTOS_DIR, exist_ok=True) # Создаем папку рядом с exe, если ее нет

COLOR_WHITE = "#FFFFFF"
COLOR_SECOND = "#F5DEB3"
COLOR_ACCENT = "#DEB887"
COLOR_DISCOUNT_ROW = "#FFDEAD"
COLOR_ZERO_STOCK = "#B2DAE7"

DISCOUNT_THRESHOLD = 17

# --- ГЛОБАЛЬНЫЙ СТИЛЬ ДЛЯ БЕЛОЙ ТЕМЫ ---
WHITE_THEME_QSS = """
    ...
    /* --- ИСПРАВЛЕНИЕ ЧЕРНЫХ РАМОК В ТАБЛИЦЕ --- */
    QHeaderView {
        background-color: #FFFFFF; 
    }
    QHeaderView::section {
...
class ProductFormDialog(QDialog):
    def __init__(self, product_id=None):
        ...
        self.photo_label.setStyleSheet("border:1px solid #467678;")
    ...
    def save_click(self):
        ...
        # Неразрывная логика замены фото: копируем новое и удаляем старое.
...
class OrderFormDialog(QDialog):
    def __init__(self, order_id=None):
        ...
        if self.order_id:
            ...
            code = DataService.get_next_pickup_code()
            self.generated_pickup_code = code
...
class ProductsWindow(QMainWindow):
    ...
    def fill_table(self, rows):
        ...
        for i, row in enumerate(rows):
            ...
            # Храним product_id в первой текстовой колонке строки.
